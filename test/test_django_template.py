...
def template_dir(temp_dir):
    ...
    django_path = templates_path / "django"
    django_path.mkdir(parents=True)
    ...
    with open(django_path / "config.yml", 'w') as f:
        yaml.dump(config, f)

    # Create some mock template files
    (django_path / "src").mkdir()
    (django_path / "src" / "api").mkdir()
    (django_path / "src" / "api" / "main.py").write_text("# ${KAVIA_TEMPLATE_PROJECT_NAME}")
    (django_path / "requirements.txt").write_text('# ${KAVIA_TEMPLATE_PROJECT_NAME}')
...
def project_config(temp_dir):
    ...
    return ProjectConfig(
        ...
        version="1.0.0",
        description="Test Django Application",
        author="Test Author",
...
def test_project_initialization(template_dir, project_config):
    ...
    assert "test-django-app" in main_content

    requirements = (output_dir / "requirements.txt").read_text()
    assert "test-django-app" in requirements
...
def test_post_processing_execution(template_dir, project_config, temp_dir):
    ...
    config_path = template_dir / "django" / "config.yml"
...
def test_template_variable_replacement(template_dir, project_config):
    ...
    test_file = template_dir / "django" / "test.txt"
    test_content = """
