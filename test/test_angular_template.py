...
from universalinit.universalinit import ProjectInitializer, TemplateProvider, AngularTemplate
...
def template_dir(temp_dir):
    ...
    angular_path = templates_path / "angular"
    angular_path.mkdir(parents=True)
    ...
    with open(angular_path / "config.yml", 'w') as f:
        yaml.dump(config, f)

    # Create some mock template files
    (angular_path / "src").mkdir()
    (angular_path / "src" / "main.ts").write_text("// ${KAVIA_TEMPLATE_PROJECT_NAME}")
    (angular_path / "angular.json").write_text('{"projects": {"${KAVIA_TEMPLATE_PROJECT_NAME}": {}}}')
    (angular_path / "package.json").write_text('{"name": "${KAVIA_TEMPLATE_PROJECT_NAME}"}')
...
def test_angular_init_info(template_dir, project_config):
    ...
    initializer.template_factory.register_template(ProjectType.ANGULAR, AngularTemplate)
...
def test_project_initialization(template_dir, project_config):
    ...
    assert (output_dir / "angular.json").exists()
    assert (output_dir / "package.json").exists()

    # Verify content replacement
    main_content = (output_dir / "src" / "main.ts").read_text()
    assert "test-angular-app" in main_content

    angular_json = (output_dir / "angular.json").read_text()
    assert "test-angular-app" in angular_json

    package_json = (output_dir / "package.json").read_text()
    assert "test-angular-app" in package_json
...
def test_post_processing_execution(template_dir, project_config, temp_dir):
    ...
    config_path = template_dir / "angular" / "config.yml"
