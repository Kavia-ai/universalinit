...
from universalinit.universalinit import ProjectInitializer, TemplateProvider, FastAPITemplate
...
def template_dir(temp_dir):
    ...
    fastapi_path = templates_path / "fastapi"
    fastapi_path.mkdir(parents=True)
    ...
    config = {
        ...
        "openapi_generation": {
            "command": "python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt && python src/api/generate_openapi.py",
            "working_directory": str(fastapi_path)
        },
        'test_tool': {
            'command': 'pytest',
    ...
    with open(fastapi_path / "config.yml", 'w') as f:
        yaml.dump(config, f)

    # Create some mock template files
    (fastapi_path / "src").mkdir()
    (fastapi_path / "src" / "api").mkdir()
    (fastapi_path / "src" / "api" / "main.py").write_text("# ${KAVIA_TEMPLATE_PROJECT_NAME}")
    (fastapi_path / "requirements.txt").write_text('# ${KAVIA_TEMPLATE_PROJECT_NAME}')
...
def test_project_initialization(template_dir, project_config):
    ...
    initializer.template_factory.register_template(ProjectType.FASTAPI, FastAPITemplate)
    ...
    assert "test-fastapi-app" in main_content

    requirements = (output_dir / "requirements.txt").read_text()
    assert "test-fastapi-app" in requirements


def test_fastapi_init_info(template_dir, project_config):
...
def test_post_processing_execution(template_dir, project_config, temp_dir):
    ...
    config_path = template_dir / "fastapi" / "config.yml"
...
def test_template_variable_replacement(template_dir, project_config):
    ...
    test_file = template_dir / "fastapi" / "test.txt"
    test_content = """
