...
from universalinit.universalinit import ProjectInitializer, TemplateProvider, SvelteTemplate
...
def template_dir(temp_dir):
    ...
    svelte_path = templates_path / "svelte"
    svelte_path.mkdir(parents=True)
    ...
    config = {
        ...
        'init_files': [],
        'init_minimal': 'Minimal Svelte application initialized',
        'run_tool': {
            'command': 'npm run dev',
            'working_directory': str(svelte_path)
        },
    ...
    with open(svelte_path / "config.yml", 'w') as f:
        yaml.dump(config, f)

    # Create some mock template files
    (svelte_path / "src").mkdir()
    (svelte_path / "src" / "routes").mkdir(parents=True)
    (svelte_path / "src" / "routes" / "+page.svelte").write_text("<h1>${KAVIA_TEMPLATE_PROJECT_NAME}</h1>")
    (svelte_path / "package.json").write_text('{"name": "${KAVIA_TEMPLATE_PROJECT_NAME}"}')
...
def project_config(temp_dir):
    ...
    return ProjectConfig(
        ...
        version="1.0.0",
        description="Test Svelte Application",
        author="Test Author",
        project_type=ProjectType.SVELTE,
        output_path=temp_dir / "output",
        parameters={
            "typescript": True,
            "styling_solution": "css"
        }
    )

def test_svelte_init_info(template_dir, project_config):
    ...
    initializer.template_factory.register_template(ProjectType.SVELTE, SvelteTemplate)
...
def test_project_initialization(template_dir, project_config):
    ...
    assert (output_dir / "src" / "routes" / "+page.svelte").exists()
    assert (output_dir / "package.json").exists()

    # Verify content replacement
    page_content = (output_dir / "src" / "routes" / "+page.svelte").read_text()
    assert "test-svelte-app" in page_content

    package_json = (output_dir / "package.json").read_text()
    assert "test-svelte-app" in package_json
...
def test_post_processing_execution(template_dir, project_config, temp_dir):
    ...
    config_path = template_dir / "svelte" / "config.yml"
